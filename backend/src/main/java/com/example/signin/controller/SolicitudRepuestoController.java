...
public class SolicitudRepuestoController {
    ...
    @GetMapping("/{id}")
    public SolicitudRepuesto obtener(@PathVariable("id") Long id) {
        return solicitudRepuestoService.obtenerPorId(id);
    }

    @GetMapping("/orden/{ordenTrabajoId}")
    public List<SolicitudRepuesto> obtenerPorOrden(@PathVariable("ordenTrabajoId") Long ordenTrabajoId) {
        return solicitudRepuestoService.obtenerPorOrden(ordenTrabajoId);
    }

    @GetMapping("/estado/{estado}")
    public List<SolicitudRepuesto> obtenerPorEstado(@PathVariable("estado") String estado) {
        return solicitudRepuestoService.obtenerPorEstado(estado);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public SolicitudRepuesto crear(@RequestBody SolicitudRepuestoDTO dto) {
